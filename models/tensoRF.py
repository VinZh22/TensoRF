...
class TensorTucker(TensorBase):
    def __init__(self, aabb, gridSize, device, **kargs):
        super(TensorTucker, self).__init__(aabb, gridSize, device, **kargs)

    def init_svd_volume(self, res, device):
        self.core_tensor_density = torch.nn.Parameter(0.2 * torch.randn((self.density_n_comp[0], self.density_n_comp[0], self.density_n_comp[0]), device=device))
        self.density_line = self.init_one_svd(self.density_n_comp[0], self.gridSize, 0.2, device)

        self.core_tensor_app = torch.nn.Parameter(0.2 * torch.randn((self.app_n_comp[0], self.app_n_comp[0], self.app_n_comp[0]), device=device))
        self.app_line = self.init_one_svd(self.app_n_comp[0], self.gridSize, 0.2, device)
        print("app_dim", self.app_dim)
        print("app_n_comp", self.app_n_comp)
        self.basis_mat = torch.nn.Parameter(torch.randn((self.app_n_comp[0], self.app_dim), device=device))
    ...
    def init_one_svd(self, n_component, gridSize, scale, device):
        ...
        for i in range(len(self.vecMode)):
            ...
            line_coef.append(
                torch.nn.Parameter(scale * torch.randn((1, n_component, gridSize[vec_id], 1))))
        return torch.nn.ParameterList(line_coef).to(device)  # size (3, 1, n_component, gridSize, rank)

    def get_optparam_groups(self, lr_init_spatialxyz=0.02, lr_init_network=0.001):
        grad_vars = [{'params': self.core_tensor_density, 'lr': lr_init_spatialxyz},
            {'params': self.density_line, 'lr': lr_init_spatialxyz},
            {'params': self.core_tensor_app, 'lr': lr_init_spatialxyz},
            {'params': self.app_line, 'lr': lr_init_spatialxyz},
            {'params': self.basis_mat, 'lr':lr_init_network}]
        if isinstance(self.renderModule, torch.nn.Module):
            grad_vars += [{'params': self.renderModule.parameters(), 'lr': lr_init_network}]
        return grad_vars
    ...
    def compute_densityfeature(self, xyz_sampled):
        """
        Compute the density feature at the sampled points
        :param xyz_sampled: Sampled points in the scene
        :return: Density feature at the sampled points
        """
        coordinate_line = torch.stack((xyz_sampled[..., self.vecMode[0]], xyz_sampled[..., self.vecMode[1]], xyz_sampled[..., self.vecMode[2]]))
        coordinate_line = torch.stack((torch.zeros_like(coordinate_line), coordinate_line), dim=-1).detach().view(3, -1, 1, 2)  #shape (3, N_point, 1, 2)
        # Compute interpolated factor values
        factor_x = F.grid_sample(self.density_line[0], coordinate_line[[0]],
                                            align_corners=True).view(-1, *xyz_sampled.shape[:1])
        factor_y = F.grid_sample(self.density_line[1], coordinate_line[[1]],
                                        align_corners=True).view(-1, *xyz_sampled.shape[:1])
        factor_z = F.grid_sample(self.density_line[2], coordinate_line[[2]],
                                        align_corners=True).view(-1, *xyz_sampled.shape[:1])
        # Perform Tucker tensor contraction to get density feature
        density_feature = torch.einsum('ijk,ib,kb,jb->b', self.core_tensor_density, factor_x, factor_y, factor_z)
    ...
    def compute_appfeature(self, xyz_sampled):
        ...
        coordinate_line = torch.stack((torch.zeros_like(coordinate_line), coordinate_line), dim=-1).detach().view(3, -1, 1, 2)  #shape (3, N_point, 1, 2)
        # Compute interpolated factor values
        factor_x = F.grid_sample(self.app_line[0], coordinate_line[[0]],
                                            align_corners=True).view(-1, *xyz_sampled.shape[:1])
        factor_y = F.grid_sample(self.app_line[1], coordinate_line[[1]],
                                        align_corners=True).view(-1, *xyz_sampled.shape[:1])
        factor_z = F.grid_sample(self.app_line[2], coordinate_line[[2]],
                                        align_corners=True).view(-1, *xyz_sampled.shape[:1])
        # Perform Tucker tensor contraction to get density feature
        app_feature = torch.einsum('ijk, ib, kb, jb, in->bn', self.core_tensor_app, factor_x, factor_y, factor_z, self.basis_mat)
        return app_feature
    ...
    def shrink(self, new_aabb):
        ...
        for i in range(len(self.vecMode)):
            mode0 = self.vecMode[i]
            print("density_line", self.density_line[i].shape)
            self.density_line[i] = torch.nn.Parameter(
                self.density_line[i].data[...,t_l[mode0]:b_r[mode0],:]
            )
            print("density_line", self.density_line[i].shape)
